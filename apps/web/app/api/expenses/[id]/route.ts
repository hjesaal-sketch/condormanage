...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${supabaseUrl}/rest/v1/expenses?id=eq.${params.id}&tenant_id=eq.${tenantId}`,
      {
        headers: {
          'apikey': supabaseKey!,
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Error al obtener gasto', details: errorText },
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Error en Supabase:', errorText);
      return NextResponse.json(
        { error: 'Error al actualizar gasto', details: errorText },
        { status: response.status }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Error en Supabase:', errorText);
      return NextResponse.json(
        { error: 'Error al eliminar gasto', details: errorText },
        { status: response.status }
      );
    }
